...
def predict_score(model: nn.Module, x01: torch.Tensor, label: int) -> float:
    """If model has softmax: log-prob, else: logit (for reference printing)."""
    model.eval()
...
def predict_prob(model: nn.Module, x01: torch.Tensor, label: int) -> float:
    ...
    x = normalize_mnist(x01.to(dev))
    out = model(x)
    probs = out if (hasattr(model, "softmax") and model.softmax is not None) else F.softmax(out, dim=1)
...
# -----------------------------
# Activation maximization (maximize SOFTMAX probability)
# -----------------------------
def optimize_image(
    model: nn.Module,
    label: int,
    ...
    for step in range(1, steps + 1):
        ...
        # Always optimize probability (apply softmax if needed)
        probs = out if (hasattr(model, "softmax") and model.softmax is not None) else F.softmax(out, dim=1)
        score = probs[0, label]  # maximize probability directly
        ...
        if print_all_probs:
            vec = probs[0].detach().cpu().numpy()
            as_str = ", ".join(f"{v:.4f}" for v in vec.tolist())
            print(f"step {step:04d} | prob[0..9]=[{as_str}] | target={label} prob={score.item():.6f}")
...
def search_l1_weight(
    ...
    for exp in range(max_exponent + 1):
        ...
        img = optimize_image(
            ...
            show_every=0, print_all_probs=False
        )
...
def analyze_l1_mad_only(csv_path: Path, save_plot: bool = True, outdir: Optional[Path] = None, z_thresh: float = 3.5) -> List[int]:
    ...
    # Raw scale MAD (use log10 if you prefer ratio sensitivity)
    vals = l1_vals
    ...
    print(f"(analyzed values):   {[round(v, 6) for v in vals.tolist()]}")
...
def parse_expected_backdoor_label(model_path: Path) -> Optional[int]:
    ...
    Works for dynamic models too. Returns None if not found.
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run L1-search (softmax-optimized) & MAD outlier detection across model folders.")
    parser.add_argument(
        "--folders",
        nargs="+",
        default=["backdoor", "clean", "dynamic"],
        choices=["backdoor", "clean", "dynamic"],
        help="Folders to process: backdoor= saved_models_backdoor_10x10, clean= clean_mnist_models, dynamic= saved_models_dynamic_backdoor_10x10"
    )
    ...
    max_exponent  = 18
    bin_iters     = 32
    use_softmax   = False   # models typically saved without softmax
    z_thresh      = 3.5
